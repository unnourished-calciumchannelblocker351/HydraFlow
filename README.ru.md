...
### Раздельное туннелирование (новое)

Российские сайты (Яндекс, ВК, Озон, Wildberries, Сбер, Госуслуги) идут **напрямую** -- без прокси. Заблокированные сайты (YouTube, Instagram, Twitter) идут **через прокси**. Можно пользоваться и тем и другим, не отключая VPN. Это критически важно, так как с 15 апреля 2026 года российские платформы обязаны блокировать пользователей VPN.

Работает автоматически во всех клиентах (v2rayNG, Hiddify, Clash, sing-box). Настройка не требуется.
...
| Раздельное туннелирование (РФ напрямую) | Да | Вручную | Вручную | Нет | Нет |
